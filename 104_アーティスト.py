...
configfile_artistlist=folder_path_moto+"/設定/設定_チーム英語リスト.txt"
...
with open(configfile_artistlist, "r", encoding="utf-8-sig") as f:
    configfile_artist = json.load(f)
team_eigo_dict= configfile_artist["team_eigo_dict"]
...
# li取得
lis = soup.select("ul.resultList li")

team_lists=[]
for li in lis:
  
    title = li.select_one(".eventName").text
    place = li.select_one(".eventPlace").text
    link = li.select_one("a")["href"]

    for artist in team_eigo_dict.keys():
        if artist in title:
            teams = []
            teams.append(title)
            teams.append(link)
